...
def test_np_dataset_basic():
    ...
    num_features = 10
    batch_size = 32
    x = np.linspace(0, dataset_length, num=dataset_length * num_features,
    ...
    assert np.all(batch[0] == expected_batch[0]) and np.all(batch[1] == expected_batch[1])
...
def test_np_dataset_split():
    ...
    dataset = data.NpDataset(x, y=y)
    # Basic split
    train, val = dataset.validation_split(split=split, shuffle=False, seed=None)
...
def test_dataset_generator_creation():
    ...
    x = np.linspace(0, dataset_length, num=dataset_length * num_features,
                    endpoint=False).reshape((dataset_length, -1))
    y = np.arange(dataset_length)
    # TODO: This is bad practice and should be removed into a seperate testable function
    # Test that inferring the steps_per_epoch and batch_size works properly
    dataset = data.NpDataset(x, y=y)
    gen = data.DatasetGenerator(dataset, steps_per_epoch=None,
                                batch_size=batch_size, shuffle=False, seed=None)
    assert gen.steps_per_epoch == steps_per_epoch
    assert gen.batch_size == batch_size
    gen = data.DatasetGenerator(dataset, steps_per_epoch=steps_per_epoch,
                                batch_size=None, shuffle=False, seed=None)
    assert gen.steps_per_epoch == steps_per_epoch
    assert gen.batch_size == batch_size
